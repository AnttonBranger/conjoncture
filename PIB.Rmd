...
```{r}
ggplot(pib_clean %>% filter(format(TIME_PERIOD, "%Y") >= 2008), aes(x = TIME_PERIOD)) +
...
pib_final_calcul <- pib_decomp_final %>%
  ...
  mutate(
    ...
    Contrib_CFm = calculer_contribution(`Consommation Ménages`, `PIB (volume)`, type = "positif"),
    Contrib_G = calculer_contribution(`Consommation Gouv (G)`, `PIB (volume)`, type = "positif"),
    Contrib_FBCF = calculer_contribution(FBCF, `PIB (volume)`, type = "positif"),
