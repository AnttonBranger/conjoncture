...
pib_q <- get_eurostat(dataset_id)
```

```{r}
ameco <- read.csv("AMECO6.csv")
ameco_pt <- ameco %>% 
  filter(COUNTRY == "Portugal",
         TITLE == "Potential gross domestic product at 2020 reference levels")
ameco_long <- ameco_pt %>%
  pivot_longer(
    cols = starts_with("X"),
    names_to = "YEAR",
...
ameco_long <- ameco_long %>% 
  select(YEAR, VALUE)

pib_clean2 <- pib_clean %>% 
  mutate(YEAR = as.numeric(format(TIME_PERIOD, "%Y"))) %>% 
  select(YEAR, values) %>% 
  left_join(ameco_long, by = "YEAR") %>% 
  mutate(VALUE = VALUE * 1000) %>% 
  mutate(output_gap = 100 * (values - VALUE) / VALUE,
         pct_gdp_growth = (values / dplyr::lag(values, 4) - 1) * 100)
```
